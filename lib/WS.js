...
const RUN_DURATION_SECONDS = parseInt(process.env.RUN_DURATION_SECONDS) || 30; // Runtime in seconds (0 for indefinite)
const SUBSCRIPTION_MODES = process.env.SUBSCRIPTION_MODES ? JSON.parse(process.env.SUBSCRIPTION_MODES) : [1,3]; // Default: LTP mode [1]
...
function attachEvents(ws) {
  ws.on('tick', (data) => {
    if (data['0'] === 'p' && data['1'] === 'o' && data['2'] === 'n' && data['3'] === 'g') {
      ...
      // Buffer data for this token, exchangeType, and mode
      dataBuffer.set(key, { ...data, timestamp: new Date().toISOString() });
      ...
      if (modesReceived.every((k) => dataBuffer.has(k))) {
        ...
        SUBSCRIPTION_MODES.forEach((m) => {
          ...
          Object.assign(combinedData, modeData); // Flatten mode data
        });
        ...
        // Clear buffer for this token and exchangeType
