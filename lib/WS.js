...
const marketData = []; // Store combined market data ticks
...
const dataBuffer = new Map(); // Buffer for all tokens, exchangeTypes, and modes
...
function attachEvents(ws) {
  ws.on('tick', (data) => {
    if (data['0'] === 'p' && data['1'] === 'o' && data['2'] === 'n' && data['3'] === 'g') {
      ...
      const symbol = instrument ? instrument.symbol.replace(/[^a-zA-Z0-9]/g, '_') : token; // Clean symbol for prefix
      const key = `${token}_${exchangeType}_${mode}`;

      // Buffer flattened data with symbol prefix
      const flattenedData = flattenObject(data, symbol);
      dataBuffer.set(key, { ...flattenedData, [`${symbol}_timestamp`]: new Date().toISOString() });

      // Check if all tokens and modes have data
      const allKeys = instruments.flatMap((inst) =>
        SUBSCRIPTION_MODES.map((m) => `${inst.token}_${inst.exchangeType}_${m}`)
      );
      if (allKeys.every((k) => dataBuffer.has(k))) {
        const combinedData = {};
        allKeys.forEach((k) => {
          const tokenData = dataBuffer.get(k);
          Object.assign(combinedData, tokenData);
        });

        // Add instrument details for each token
        instruments.forEach((inst) => {
          const symbol = inst.symbol.replace(/[^a-zA-Z0-9]/g, '_');
          Object.assign(combinedData, flattenObject(inst, symbol));
        });

        console.log('Market data:', JSON.stringify(combinedData));
        marketData.push(combinedData);

        // Clear buffer
        allKeys.forEach((k) => dataBuffer.delete(k));
      }
    }
  });
...
// Step 11: Export main function and data
module.exports = { main, marketData, pongData };