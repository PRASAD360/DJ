...
const SUBSCRIPTION_MODES = process.env.SUBSCRIPTION_MODES ? JSON.parse(process.env.SUBSCRIPTION_MODES) : [3]; // Default: LTP mode [1]
...
function attachEvents(ws) {
  ws.on('tick', (data) => {
    if (data['0'] === 'p' && data['1'] === 'o' && data['2'] === 'n' && data['3'] === 'g') {
      ...
      // Buffer flattened data for this token, exchangeType, and mode
